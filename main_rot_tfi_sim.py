...
def main(max_iter, phi=0., hamiltonian_type=None, p=5, n=4, start_in_x=True, rot_H=False):
    ...
    for i in range(1,max_iter+1):
        ...
        fb, fb1, fb2 = opt.simulate_fubini_metric(params, draw=True)
        # regularize
...
for l in range(iters):
    ...
    np.save('saves/SIM_TFI_rot_with_ng_scores', np.array(scores))
    np.save('saves/SIM_TFI_rot_with_ng_old_grad_norm', np.array(old_grad_norm_data))
    np.save('saves/SIM_TFI_rot_with_ng_new_grad_norm', np.array(new_grad_norm_data))
    np.save('saves/SIM_TFI_rot_with_ng_parallel_norm', np.array(p_norm_data))
    np.save('saves/SIM_TFI_rot_with_ng_orthogonal_norm', np.array(o_norm_data))
...
plt.tight_layout()
plt.savefig('saves/transverse_ising_t_0_0_init_0_degrees_rotation_6_qubits_4_layers_with_ng_SIMULATED.png', dpi=400)
