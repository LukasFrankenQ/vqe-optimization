...
def main(max_iter, phi=0., hamiltonian_type='rot_single_qubit_z', p=5, n=4, start_in_x=True):
    ...
    for _ in range(p):
        ...
        #circuit.add_layer('xx', 'coll')
        #circuit.add_layer('yy', 'coll')
...
def main(max_iter, phi=0., hamiltonian_type='rot_single_qubit_z', p=5, n=4, start_in_x=True):
    ...
    """available: sk, single_qubit_z, rot_single_qubit_z, transverse_ising, spin_chain"""
    H = Hamiltonian(n, hamiltonian_type=hamiltonian_type, t=t, init_circuit=init_circuit,
                                                               append_circuit=append_circuit)
    opt = Optimizer(circuit=circuit, fubini_reps=fubini_reps, grad_reps=grad_reps, rot_circuit=True, n=n)
...
f, axs = plt.subplots(5, 1, figsize=(20, 10), sharex=True)
sns.despine(left=True)
sns.set_style('darkgrid')
...
plt.tight_layout()
plt.savefig('saves/transverse_ising_t_0_0_init_0_degrees_rotation_with_ng.png', dpi=400)
plt.show()
