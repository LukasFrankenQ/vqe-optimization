...
block_sizes = [1, int(3*n + 1), int(p + 3*n*p), 5, 8, 10, 20]
reps = 0
...
for block_size in block_sizes:
    ...
    for _ in range(runs):
        ...
        for i in range(iters):
            ...
            if fubini:
                fb, fb1, fb2 = opt.linear_time_fubini(params, blockwise=blockwise, diagonal_only=diagonal_only, block_size=block_size)
        ...
        path = 'saves/blocksize_sweep/'
        filename = 'tfi_'+str(n)+'_qubits_'+str(p)+'_layers_fubini_'+str(fubini)+"_rot_"+str(rot)+'_reg_param_'+str(reg_param)+'_blocksize_'+str(block_size)+'_'
