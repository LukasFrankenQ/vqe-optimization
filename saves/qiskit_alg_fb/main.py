...
for angle in angles:
    ...
    for _ in range(runs):
        ...
        for i in range(iters):
            ...
            if fubini:
                fb, fb1, fb2 = opt.simulate_fubini_metric(params, angle=angle)
        ...
        np.save('saves/qiskit_alg_fb/tfi_'+str(n)+'_qubits_'+str(p)+'_layers_fubini_'+str(fubini)+"_rot_"+str(rot)+'_angle_'+str(round(angle,3))+'_init_noise_'+str(init_noise)+'_scores', np.array(score_data))
        np.save('saves/qiskit_alg_fb/tfi_'+str(n)+'_qubits_'+str(p)+'_layers_fubini_'+str(fubini)+"_rot_"+str(rot)+'_angle_'+str(round(angle,3))+'_init_noise_'+str(init_noise)+'_grad_pre_norm', np.array(norm_pre_data))
        np.save('saves/qiskit_alg_fb/tfi_'+str(n)+'_qubits_'+str(p)+'_layers_fubini_'+str(fubini)+"_rot_"+str(rot)+'_angle_'+str(round(angle,3))+'_init_noise_'+str(init_noise)+'_grad_post_norm', np.array(norm_post_data))
